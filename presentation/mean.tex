...
\begin{standaloneframe}[label=current]{Mean Payoff}{Finite Game}
    \scalebox{0.7}{\MeanPayoffTree{23}}
    \scalebox{0.7}{\MeanPayoffGame}


    \bigskip
    \visible<.(6),.(8)->{ \alt<.(6)>{Max}{Min} can ensure \alt<.(6)>{$\geq$}{$\leq$} 0 \visible<.(9)->{\alert<.(9)>{in the mean payoff game too}}}
    \bigskip

    \addtocounter{beamerpauses}{9}
    \splitCol{0.45}{0.5}{
       \begin{overlayarea}{0.95\textwidth}{4em}
        ...
        \end{overlayarea}
            ...
            \begin{itemize}
                ...
                \item<only@+> Hence limsup of averages of $\pi$ is $\leq 0$
                \item<visible@+-> Similarly Max can ensure liminf of the average is $\geq 0$
                \item<visible@.> Hence the value of Mean payoff game is 0
            \end{itemize}
            }
\end{standaloneframe}
\end{document}
